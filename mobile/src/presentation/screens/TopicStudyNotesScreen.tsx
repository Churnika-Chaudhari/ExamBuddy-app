...
function isLocalTemplateNote(note: {
  ai_metadata?: {
  ...
  return (
    ...
    Boolean(meta.generation_error || meta.ai_error)
  );
}
...
function providerLabel(note: {
  ...
  if (meta?.generation_error || meta?.ai_error) {
    return String(meta.generation_error || meta.ai_error);
  }
  if (!p || p === 'local' || meta?.generation_mode === 'local_fallback') {
    return 'Notes were not generated by Gemini — tap Regenerate after fixing GEMINI_API_KEY';
  }
...
export default function TopicStudyNotesScreen() {
  ...
  const localTemplate = isLocalTemplateNote(note);
  const hasNotes = Boolean(note.notes?.trim()) && !localTemplate;

  if (localTemplate) {
    return (
      <View style={styles.centered}>
        <Ionicons name="alert-circle-outline" size={48} color={colors.error} />
        <Text style={styles.errorTitle}>AI notes unavailable</Text>
        <Text style={styles.errorText}>
          {providerLabel(note) ||
            'Gemini did not generate notes. Check GEMINI_API_KEY on the server, then Regenerate.'}
        </Text>
        <AppButton
